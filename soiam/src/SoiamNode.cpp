...
SoiamNode::SoiamNode()
{
	...
	pidcmd.gaz = pidcmd.pitch = pidcmd.roll = pidcmd.yaw = 0;
	...
	for(int b=0;b<JIT_m;b++){
		pXbuf[b] = pYbuf[b] = 0;
	}
	for(int b=0;b<JIT_p;b++){
		rXbuf[b] = rYbuf[b] = 0;
	}
	for(int b=0;b<JIT_n;b++){
		pcXbuf[b] = pcYbuf[b] = 0;
	}
...
void SoiamNode::PIDCb(const geometry_msgs::TwistConstPtr PIDPtr)
{
	geometry_msgs::TwistStamped ts;
	ts.header.stamp = ros::Time::now();
	ts.twist = *PIDPtr;

	pidcmd.pitch = ts.twist.linear.x;
	pidcmd.roll = ts.twist.linear.y;
	pidcmd.gaz = ts.twist.linear.z;
	pidcmd.yaw = ts.twist.angular.z;
...
void SoiamNode::Loop()
{
	...
	Yrowsnode =rowsnode;
	//recordsoinndata(1,Xsoinn,Xrowsnode);
	//recordsoinndata(2,Ysoinn,Yrowsnode);
	...
	while (nh_.ok())
	{
		...
		if(isLearning)
		{
			...
			randomCMD.yaw = pidcmd.yaw;
			sendControlToDrone(randomCMD);
	
			//cout << "randomX : " << randomX << ",    randomY : " << randomY << endl;
			//cout << "PIDcmd.pitch : " << pidcmd.pitch << ",    PIDcmd.roll : " << pidcmd.roll << endl;
			...
			cXbuf[JIT_n+JIT_p-1] = randomX; //learningSet.xlin;	//randomCMD.pitch
			Ybuf[JIT_m+JIT_p-1] = learningSet.ypos;
			cYbuf[JIT_n+JIT_p-1] = randomY; //learningSet.ylin;	//randomCMD.roll
		...
		if(isPredicting)
		{
			...
			pcXbuf[JIT_n-1] = Pvx; //learningSet.xlin;	//soiamCMD.pitch
			pcYbuf[JIT_n-1] = Pvy; //learningSet.ylin;	//siamCMD.roll
			...
			Pvx = soiamPrediction(Xsoinn, PXsignal, Xrowsnode, k);
			soiamCMD.pitch = Pvx + pidcmd.pitch;
			...
			// TODO:recall from SOIAM
			Pvy = soiamPrediction(Ysoinn, PYsignal, Yrowsnode, k);
			soiamCMD.roll = Pvy + pidcmd.roll;

			soiamCMD.gaz = Pvz = pidcmd.gaz;
			soiamCMD.yaw = Pvr = pidcmd.yaw;
			sendControlToDrone(soiamCMD);

			//test
			/*
			for(int i=0;i<Dim-1;i++){
				cout << "PXsignal[" << i << "] = " << PXsignal[i] << "               " << "PYsignal[" << i << "] = " << PYsignal[i] << endl;
			}
			*/

	
			//std::cout << "Pvx : " << Pvx << ",    Pvy : " << Pvy << endl;
			//cout << "PIDcmd.x : " << pidcmd.pitch << ",    PIDcmd.y : " << pidcmd.roll << endl;
			...
			//record Position & PIDCommand
			//recFile3(learningSet.xpos,learningSet.ypos,pidcmd.pitch,pidcmd.roll);
			//record Position & (Soiam+PID)Command
			//recFile4(learningSet.xpos,learningSet.ypos,soiamCMD.pitch,soiamCMD.roll);
...
double SoiamNode::soiamPrediction(CSOINN *csoinn, double *signal, int rowsnode, int k){
	...
	it=data.begin();
	//inverse sum
	double x = 0.01;//if dist=0 then dist=x
	for(int i=0;i<k;i++){
		if(it->first==0){
			inversesum += 1/x;
		}else{
		inversesum += 1/it->first;
		}
		//cout << "dist : "<< it->first << endl; //test
		//cout << "inversesum : "<< inversesum << endl; //test
		it++;
	}
	//kNN
	double value = 0.0;
	...
	for(int i=0;i<k;i++){
		if(it->first==0){
			value += ((1/x)/inversesum) * csoinn->GetNode(it->second)->GetSignal()[Dim-1];//cmd[t+1]
		}else{
			value += ((1/it->first)/inversesum) * csoinn->GetNode(it->second)->GetSignal()[Dim-1];//cmd[t+1]
		}
		//cout << "cmd(t+1) : " <<csoinn->GetNode(it->second)->GetSignal()[Dim-1] << endl;
...
void SoiamNode::recFile3 (double posX, double posY, double PIDx, double PIDy) 
{
	ofstream rec;
  	rec.open ("/home/enomoto/catkin_ws/src/soiam/rec/Position_cmd(PID).txt", ios::out | ios::app);
  	rec << posX << " " << posY  << " " << PIDx << " " << PIDy << endl;
  	rec.close();
}

void SoiamNode::recFile4 (double posX, double posY, double vx, double vy) 
{
	ofstream rec;
  	rec.open ("/home/enomoto/catkin_ws/src/soiam/rec/Position_cmd.txt", ios::out | ios::app);
  	rec << posX << " " << posY  << " " << vx << " " << vy << endl;
  	rec.close();
}
...
double SoiamNode::loadTrainingdata(int a, CSOINN *csoinn){
	...
	if (rec.is_open()) {
		...
		for(int i=0;i<cols*rows;i++){
			if(s<JIT_m+JIT_p){
				if(10>fabs(sdata[i])&&fabs(sdata[i])>maxpos){
					maxpos=fabs(sdata[i]);
				}
			}
			s++;
...
double SoiamNode::RandomInput(double min, double max){
	static int flag;
	if(flag==0){
...
void SoiamNode::recordsoinndata(int a, CSOINN *csoinn, int rowsnode){
	// Open txt file 
	ofstream rec;
	if(a==1){
  	rec.open ("/home/enomoto/catkin_ws/src/soiam/rec/XSOINN.txt", ios::out | ios::app);
	}else if(a==2){
  	rec.open ("/home/enomoto/catkin_ws/src/soiam/rec/YSOINN.txt", ios::out | ios::app);
	}
	
	//record SOINN data 
	for(int i=0;i<rowsnode;i++){
		for(int j=0;j<Dim;j++){
			rec << csoinn->GetNode(i)->GetSignal()[j] << " " << flush;
		}
		rec << endl;
	}
	rec.close();
